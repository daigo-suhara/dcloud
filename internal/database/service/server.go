...
func (s *Server) ListDatabases(ctx context.Context, req *ListDatabasesRequest) (*ListDatabasesResponse, error) {
	...
	records, err := s.Kube.listDatabases(ctx, projectID, userID, projectID)
	if err != nil {
...
func (s *Server) CreateDatabase(ctx context.Context, req *CreateDatabaseRequest) (*CreateDatabaseResponse, error) {
	userID := strings.TrimSpace(req.UserId)
	projectID := strings.TrimSpace(req.ProjectId)
	...
	record, err := s.Kube.createDatabase(ctx, projectID, userID, projectID, name, dbType, version, cpu, memory, storageSize, s.StorageClass)
	if err != nil {
		if errors.Is(err, errUnavailable) {
			return nil, status.Error(codes.FailedPrecondition, err.Error())
		}
...
func (s *Server) DeleteDatabase(ctx context.Context, req *DeleteDatabaseRequest) (*DeleteDatabaseResponse, error) {
	userID := strings.TrimSpace(req.UserId)
	projectID := strings.TrimSpace(req.ProjectId)
	name := strings.TrimSpace(req.Name)
	...
	go func() {
		...
		if err := s.Kube.deleteDatabase(bgCtx, projectID, resourceName); err != nil {
...
func (s *Server) GetDatabase(ctx context.Context, req *GetDatabaseRequest) (*GetDatabaseResponse, error) {
	userID := strings.TrimSpace(req.UserId)
	projectID := strings.TrimSpace(req.ProjectId)
	name := strings.TrimSpace(req.Name)
	if userID == "" || projectID == "" || name == "" {
...
func (s *Server) GetConnectionString(ctx context.Context, req *GetConnectionStringRequest) (*GetConnectionStringResponse, error) {
	userID := strings.TrimSpace(req.UserId)
	projectID := strings.TrimSpace(req.ProjectId)
	name := strings.TrimSpace(req.Name)
	if userID == "" || projectID == "" || name == "" {
	...
	connInfo, err := s.Kube.getConnectionString(ctx, found.ProjectID, found, strings.TrimSpace(req.SchemaName))
	if err != nil {
...
func (s *Server) ListSchemas(ctx context.Context, req *ListSchemasRequest) (*ListSchemasResponse, error) {
	rec, err := s.lookupInstance(ctx, req.UserId, req.ProjectId, req.Name)
	if err != nil {
		return nil, err
	}
	names, err := s.Kube.listSchemas(ctx, rec.ProjectID, rec)
	if err != nil {
...
func (s *Server) CreateSchema(ctx context.Context, req *CreateSchemaRequest) (*CreateSchemaResponse, error) {
	rec, err := s.lookupInstance(ctx, req.UserId, req.ProjectId, req.Name)
	if err != nil {
		return nil, err
	}
	...
	if err := s.Kube.createSchema(ctx, rec.ProjectID, rec, schemaName, strings.TrimSpace(req.Charset)); err != nil {
		if errors.Is(err, errInvalidArgument) {
			return nil, status.Error(codes.InvalidArgument, err.Error())
		}
...
func (s *Server) DeleteSchema(ctx context.Context, req *DeleteSchemaRequest) (*DeleteSchemaResponse, error) {
	rec, err := s.lookupInstance(ctx, req.UserId, req.ProjectId, req.Name)
	if err != nil {
		return nil, err
	}
	...
	if err := s.Kube.deleteSchema(ctx, rec.ProjectID, rec, schemaName); err != nil {
		if errors.Is(err, errInvalidArgument) {
			return nil, status.Error(codes.InvalidArgument, err.Error())
		}
	...
	for {
		...
		case <-ticker.C:
			s.reconcileResourceType(ctx, "database", func(op dbsqlc.ListPendingOperationsByResourceTypeRow) bool {
				...
				records, err := s.Kube.listDatabases(ctx, op.ProjectID.String, op.UserID.String, op.ProjectID.String)
				if err != nil {
					return false
				}
				for _, r := range records {
